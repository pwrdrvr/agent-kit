...
- 384493a: Make a low-effort reasoning request actually lower an ACP agent's thinking when that agent offers only graded levels, and choose the level from the session's own option menu rather than whichever session wrote last.
...
- a91fb4f: Fix ACP model selection, which threw before the request ever left the process, and route it through the config option when that is how the agent advertises model choice.
...
- ccd0e63: ACP: drop unparseable `config_option_update` notifications instead of erroring
...
- 01fa6dd: ACP: send `session/set_config_option` without the `_` extension prefix
