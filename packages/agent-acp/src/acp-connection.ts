...
// library's typed methods (with generic `request` / `notify` for non-standard
// ones like `session/set_config_option`); the library's `Client` callbacks
...
export class AcpConnection implements AcpJsonRpcTransport {
  ...
  private defaultCreateConnection(): (client: Client) => AcpAgentConnectionHandle {
    ...
    return (client) => {
      ...
      // SDK 1.3 accepts the protocol's scalar/array tool raw values and config
      // option updates directly, so passing stdio through unmodified preserves
      // the agent's exact notification payload.
      ...
      // ClientSideConnection is deprecated in favor of the SDK's typed
      // client() builder, but it is the supported compatibility bridge that
      // retains wildcard inbound extension handlers. Agent-kit must preserve
      // unknown vendor request/notification routing for discovery strategies;
      // the typed builder requires registering each custom method up front.
  ...
  async request(
    ...
    switch (method) {
      ...
      default:
        // Unknown vendor methods remain part of agent-kit's generic transport
        // seam. SDK 1.3 sends them verbatim through its non-deprecated request().
